...
`docs/glossaries.html` from `glossaries/*.md`, each through its
`docs/*.template.html`, and fails when a page differs from the committed one:
edit the markdown or the template, never the page, then run `bin/build`.
